    ...
    <div className={` ${className}`}>
      <div className='flex flex-wrap items-center alert alert-success m-2 p-1'>
        <div className='w-2/12'>
          <Image src={'/img/Chumaru_left.png'} alt='' width={40} height={60} />
        </div>
        <div className='w-10/12'>
          <p className=' font-bold my-4 text-left text-sm   '>{props.questionText}</p>
        </div>
          ...
          return (
            <div className='p-1   ' key={index}>
              <button
                ...
                style={{ minWidth: 80 }}
                className={` rounded-md p-2 w-60 font-bold bg-yellow-400 text-dark ${
                  questions[questionNumber].answer === option ? 'opacity-100' : 'opacity-40'
                }`}
                onClick={addAnswer}
              >
                {option}
        ...
        <div className='flex flex-row space-x-8 justify-around my-4'>
          ...
          <BtnBasic
            className={`bg-blue-400 text-white ${
              questions[questionNumber].answer == '' ? 'pointer-events-none opacity-30' : ''
            }`}
            onClick={addStage}
          >
            次へ
