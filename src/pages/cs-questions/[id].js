...
export default function QuestionHome() {
  const { stage, setstage, addStage, reduceStage, stageClass, questions, setquestions, customerId, setcustomerId } =
  ...
  const test = () => {
    const res = fetch('/api/google-sheets-api')
      .then((res) => res.json())
      .then((data) => {
        console.log(data); //////////
      })
    ...
    <Layout>
      ...
      <div className='p-2'>
        ...
        {answersCount === questions.length && (
          <BtnBasic onClick={() => setstage(questions.length)}> 全ての回答を確認</BtnBasic>
        )}
      </div>
    </Layout>
  );
